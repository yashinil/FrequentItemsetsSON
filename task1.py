...
#python first1.py 1 4 small2.csv firstoutput.txt
...
smallRDD=sc.textFile(input_path).map(lambda line: line.split(',')).filter(lambda line: True if(line[0]!='user_id') else False)
if case_no==1:
    smallRDD=smallRDD.map(lambda line: (line[0],[line[1]]))
else:
    smallRDD=smallRDD.map(lambda line: (line[1],[line[0]]))

#making user/business baskets
basketsRDD=smallRDD.reduceByKey(lambda a,b: a+b).persist(pyspark.StorageLevel.DISK_ONLY)
