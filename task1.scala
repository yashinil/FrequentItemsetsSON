...
object task1 {
  def main(args: Array[String]): Unit = {
    ...
    val case_no=args(0).toInt
    val threshold=args(1).toInt
    val input_path=args(2)
    val output_path=args(3)

    val smallRDD = sc.textFile(input_path).map(line=>line.split(',')).filter(line=> line(0)!="user_id")

    type pairRDD=(String,List[String])
    var basketsRDD=sc.emptyRDD[pairRDD]
    if(case_no==1){
      basketsRDD = smallRDD.map(line=> (line(0),List(line(1)))).reduceByKey((a,b)=>a++b).persist(StorageLevel.DISK_ONLY)
    }
    else{
      basketsRDD = smallRDD.map(line=> (line(1),List(line(0)))).reduceByKey((a,b)=>a++b).persist(StorageLevel.DISK_ONLY)
    }
    ...
    def apriori(iterator: Iterator[pairRDD]): Iterator[pair] = {
      ...
      var reduced_threshold=((baskets_in_partition*1.0/total_baskets)*threshold*1.0).floor
